...
// Function to print the contents of an unordered_set
template <typename T>
void printUnordered_set(const unordered_set<T> &container)
{
    cout << "{";
...
template <typename T>
void printBuckets(const unordered_set<T> &container, int bucketStart, int bucketEnd)
{
...
int main()
{
    unordered_set<int> nums = {4, 3, 2, 1, 4, 1, 5, 2}; // Duplicate values will be removed automatically
    unordered_set<int> oddNums = {1, 3, 5, 7, 9, 11};

    // Print all elements in 'nums'
    cout << "nums:\n";
    printUnordered_set(nums);
