...
void printUnordered_map(const unordered_map<key, T> &container)
{
    cout << "{";
...
template <typename key, typename T>
void printBuckets(const unordered_map<key, T> &container, int bucketStart, int bucketEnd)
{
...
{
    unordered_map<int, string> nums = {{4, "four"}, {3, "three"}, {2, "two"}, {1, "one"}, {5, "five"}};
    unordered_map<int, string> oddNums = {{1, "one"}, {3, "three"}, {5, "five"}, {7, "seven"}, {9, "nine"}, {11, "eleven"}};
    ...
    unordered_map<int, string>::const_iterator end = nums.find(2); // The end of range
